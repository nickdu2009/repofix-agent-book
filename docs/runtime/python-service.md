...
Python 服务只拥有一次 Run 内部的模型决策循环。Go 传入 Run、任务和短期 Tool Gateway capability，Python 使用 HTTP Client 回调 Go 的受限工具接口，再返回结构化候选结果。当前闭环尚未上报 Step 事件；事件上报要等事件端口接入后实现。
...
!!! success "当前伴随实现"
    `services/agent` 已提供可运行的 FastAPI 边界和使用标准库 HTTP 的 `ToolGatewayClient`。默认应用故意 fail-closed：未注入 RunnerFactory 时 `/healthz` 为 200、`/readyz` 为 503，也不会在 import 时创建 OpenAI Client。
...
## Python 回调 Go Tool Gateway

`ToolGatewayClient` 实现现有 `ToolExecutor` 协议，不改变 Agent Loop。RunnerFactory 使用当前请求中的 URL 和 capability 创建它：

```python
executor = ToolGatewayClient(
    str(request.tool_gateway_url),
    request.workspace_capability.get_secret_value(),
    request.initial_workspace_revision,
)
```
...
{"tool":"run_tests","arguments":{"target":"unit"},"timeout_ms":30000}
```

Go 同时传入 Sandbox 当前的 `initial_workspace_revision`，避免非零基线被误判为 Agent 修改。Client 严格校验后续响应，并使用 Go 返回的 `workspace_revision` 更新本地状态。Client 禁止 HTTP 重定向，网络、HTTP 和非法响应统一抛出 `ToolGatewayError`，服务边界稳定返回 `tool_gateway_failed`。Client 不接收 Daytona 凭据，也不允许模型提供 Tool Gateway URL 或 capability；这些值只能由 Go 的 Run 请求给出。
...
当前 Tool Gateway 异常已经稳定映射为 `tool_gateway_failed`。接入 Live Model 组合根后，还必须补齐 `model_rate_limited` 和 `model_timeout`；当前实现不能宣称已经提供这两类模型错误。
...
预期：API 和 Tool Gateway Client 测试都不调用网络；Python 与 Go 测试共同读取 `contracts/fixtures/` 的请求和响应样例，防止两端 DTO 漂移。直接启动的默认应用用于观察 fail-closed 行为：`/healthz` 返回进程存活，`/readyz` 返回顶层 `not_ready` 错误。真正 ready 的进程必须由组合根注入 Live Model 与 `ToolGatewayClient`；只有 Go Gateway 已连接真实隔离 Sandbox 时才能使用 Live Model。
...
当前验收覆盖：不就绪、Fake 完整闭环、重复 `run_id`、非法 Schema、稳定的 Gateway 失败，以及同步模型调用期间取消 endpoint 仍可响应。接入 Live Adapter 前还必须补模型超时、限流和真实服务身份验证合约测试。
